...
var Filter = new FilterObject.ProfanityFilter(profanityFile);
...
var configData = { prefix, logChannelID, permissionMap }
    = getConfigData();
...
client.on("message", message => {
    ...
    if (!command) {
        return message.reply(`I'm sorry, I don't have any command called ${commandName}!`);
    }
    ...
    } // If we don't have a most recent usage, set now as the most recent
    else {
    ...
    if (command.requiredArgs > commandArgs.length || command.maxArgs < commandArgs.length) {
        return message.reply(`You did not provide the correct number of arguments for the ${command.name} command!`
...
function getConfigData() {
    var newPrefix, newLogChannelID, newConfigData, newPermissionMap;

    // Reload data from config file
    newConfigData = JSON.parse(fs.readFileSync('./configuration/config.json'));
    newPrefix = newConfigData["prefix"];
    newLogChannelID = newConfigData["logChannelID"];

    // Reload data from permission map
    newPermissionMap = JSON.parse(fs.readFileSync('./configuration/permissions_map.json'));
